...
void handleCGI(RequestCgi &request, ResponseCgi &response)
{
    ...
    { // Parent process
        ...
        {
            std::cout << "here91111111111111111111" << std::endl;
...
{
    RequestCgi req_cgi(Method::toString(req._method),             // Method name
                       req._location_res["root"] + req._uri.host, // Script Path
                       req._uri.query,                            // Query_string
                       req._content_type,                         // Content_length (NO Need for GET)
                       req._headers["content-length"],            // Content_Type   (NO Need for GET)
                       req._tmp_file_name.empty()
                           ? "./www/tmp/tmp"
                           : req._tmp_file_name, // Body as file
